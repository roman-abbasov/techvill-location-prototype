...
export function buildHeatmapData(scores, zonesById, samplesPerZone = 22) {
  ...
  points.forEach((point) => {
    const weight = 0.28 + 0.72 * ((point.score - minScore) / range);
    features.push({ type: 'Feature', id: point.id, geometry: { type: 'Point', coordinates: [point.zone.center_lat, point.zone.center_lng] }, properties: { weight, source: true } });
    for (let index = 0; index < samplesPerZone; index += 1) {
      const radialShare = Math.sqrt((index + 0.5) / samplesPerZone);
      const radius = maxRadius * radialShare;
      const angle = index * goldenAngle;
      const lat = point.zone.center_lat + Math.cos(angle) * radius;
      const lng = point.zone.center_lng + (Math.sin(angle) * radius) / 0.56;
      features.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [lat, lng] }, properties: { weight: weight * (1 - 0.65 * radialShare), source: false } });
    }
  });
...
export default function PotentialMap({ apiKey, zones, stores, scores, selectedZoneId, onSelectZone }) {
  ...
  useEffect(() => {
    ...
    ymaps.modules.require(['Heatmap'], (Heatmap) => { if (!mapRef.current) return; const heatmap = new Heatmap(buildHeatmapData(scores, zonesById), { radius: 30, dissipating: true, opacity: 0.72, intensityOfMidpoint: 0.32, gradient: { 0.1: 'rgba(80,210,85,.32)', 0.25: 'rgba(80,210,85,.55)', 0.5: 'rgba(235,230,68,.68)', 0.72: 'rgba(250,157,47,.78)', 1: 'rgba(226,62,48,.88)' } }); heatmap.setMap(map); layersRef.current.push(heatmap); });
